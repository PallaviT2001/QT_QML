...
QString Contact::callTime() const { return m_callTime; }
bool Contact::isIncoming() const { return m_isIncoming; }
...
void Contact::setNumber(const QString &number) {
    if (m_number != number) { m_number = number; emit numberChanged(); }
}
void Contact::setImage(const QString &image) {
    if (m_image != image) { m_image = image; emit imageChanged(); }
}
void Contact::setCallTime(const QString &time) {
    if (m_callTime != time) { m_callTime = time; emit callTimeChanged(); }
}
void Contact::setIsIncoming(bool incoming) {
    if (m_isIncoming != incoming) { m_isIncoming = incoming; emit isIncomingChanged(); }
}
void Contact::setIsOutgoing(bool outgoing) {
    if (m_isOutgoing != outgoing) { m_isOutgoing = outgoing; emit isOutgoingChanged(); }
}
void Contact::setShortMessage(const QString &msg) {
    if (m_shortMessage != msg) { m_shortMessage = msg; emit shortMessageChanged(); }
}
